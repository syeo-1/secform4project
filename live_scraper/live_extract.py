...
def scrape_for_SEC_form(form_type, items_per_page, start_time=None, end_time=60):
    ...
    midnight_est = datetime.combine(est.date(), dtime.min, tzinfo=ZoneInfo('America/New_York'))


    all_formatted_text_links = []

    # looks like I can only go back to at most 2000 filings anyways (entering 2100 seems to give no data)
    for page_start in range(0, 2100, 100):
        ...
        for tr_link_element in tr_link_elements:
            ...
            # formatted datetime is already in EST since filing entries will be in EST, so no extra conversions necessary
            formatted_datetime = formatted_datetime.replace(tzinfo=ZoneInfo('America/New_York'))

            if formatted_datetime < midnight_est:
                # searching beyond midnight on the current date. If that's the case, just use the historical scraper
